...
export default function Products() {
  ...
  const handleAdjust = async (product) => {
    ...
    if (isNaN(adj) || adj === 0) return setAdjustError('Enter a non-zero number (e.g. +10 or -5)')
    try {
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        {products.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-gray-500 font-medium">{search ? 'No products found' : 'No products yet'}</p>
            {!search && (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  {['Name', 'SKU', 'Quantity', 'Status', 'Selling Price', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {products.map(product => (
                  <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-mono text-gray-500 text-sm">{product.sku}</td>
                    <td className="px-6 py-4">
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              placeholder="+/-"
                              className="w-20 px-2 py-1.5 border border-indigo-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                              autoFocus
                            ...
                            <button onClick={() => setAdjusting(null)} className="text-xs text-gray-400 hover:underline">Cancel</button>
                          </div>
                          {adjustError && <p className="text-xs text-red-500">{adjustError}</p>}
                          <p className="text-xs text-gray-400">Current: {product.quantity_on_hand}</p>
                        </div>
                      ) : (
