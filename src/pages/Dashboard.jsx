        ...
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Products at or below their threshold (default: {data.defaultThreshold})
            </p>
          </div>
          <Link to="/products" className="text-sm text-indigo-600 hover:text-indigo-800 font-medium">
            View all products →
          </Link>
        </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.lowStockItems.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-gray-800 text-sm">{item.name}</td>
                    ...
                    <td className="px-6 py-4 text-gray-500 text-sm">
                      {item.low_stock_threshold != null ? item.low_stock_threshold : `${data.defaultThreshold} (default)`}
                    </td>
                    <td className="px-6 py-4">
                      <Link
